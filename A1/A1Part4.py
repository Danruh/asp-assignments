...
def downsampleAudio(inputFile, M):
    ...
    """
    ## Your code here
    fs, x_raw = wavread(inputFile)

    x_downsampled = hopSamples(x_raw, M)
    fs_new = round(fs / M)

    new_filename = inputFile.split('/')[-1].split('.wav')[0]
    wavwrite(x_downsampled, fs_new, f'{new_filename}_downsampled.wav')


def main():
    downsampleAudio('../../sounds/vibraphone-C6.wav', 100)
