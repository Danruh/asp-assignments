...
def DFT(x):
    ...
    # Create an NxN Fourier matrix such that DFT(x) = Sx
    # where the rows correspond do the kth term and the columns
    # correspond to the nth term
    for row, col in np.ndindex(S.shape):
        k = row
        n = col
        S[row, col] = np.exp(-1j*2*np.pi*k*n/N)

    return S @ x


def main():
    F = DFT(np.array([1, 2, 3, 4]))
